...
int callback(int message) {
  ...
  case SOLLYA_MSG_MIN_RELIES_ON_FP_RESULT_FAITHFUL_BUT_UNDECIDED:
    sollya_lib_printf("The following test is not certified (probably two equal minimal values at some point of the algorithm).\n");
    break;
  ...
  case SOLLYA_MSG_EXPR_SHOULD_BE_CONSTANT_AND_IS_NOT_FAITHFUL:
    sollya_lib_printf("One of the terms of the following test is hard to evaluate (probably min=0 at some point of the algorithm).\n");
    break;
