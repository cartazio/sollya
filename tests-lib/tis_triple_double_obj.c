...
int main(void) {
  ...
  for(i=0;i<53;i++)  t[i] = sollya_lib_is_triple_double_obj(a[i]);
  ...
  if (t[50]) sollya_lib_printf("sollya_lib_is_triple_double_obj detects structure { .a = 5; .b = exp(_x_)}.\n");
  if (t[51]) sollya_lib_printf("sollya_lib_is_triple_double_obj detects procedure proc(a) { return a+1; }.\n");
  if (t[52]) sollya_lib_printf("sollya_lib_is_triple_double_obj detects external procedure based on empty_func.\n");
