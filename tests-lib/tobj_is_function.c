...
int main(void) {
  ...
  for(i=0;i<53;i++)  t[i] = sollya_lib_obj_is_function(a[i]);
  ...
  if (t[50]) sollya_lib_printf("sollya_lib_obj_is_function detects structure { .a = 5; .b = exp(_x_)}.\n");
  if (t[51]) sollya_lib_printf("sollya_lib_obj_is_function detects procedure proc(a) { return a+1; }.\n");
  if (t[52]) sollya_lib_printf("sollya_lib_obj_is_function detects external procedure based on empty_func.\n");
