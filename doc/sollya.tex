...
\end{minipage}
\end{center} 
That construct has the following semantic: try to match the given
...
important to realize the meaning of programming-language-level
variables in patterns. This meaning is different from the one usually
found for variables. In a pattern, variables are never evaluated to
whatever they might have set before the pattern is executed. In
contrast, all variables in patterns are new, free variables that will
freshly be bound to subexpressions of the matching expression. If a
variable of the same name exists already, it will be shadowed during
the evaluation of the statement block and return expression that get
...
As a matter of course, all variables figuring in the expression {\it
  expr} to be matched (indicated as \key{match {\it expr} with}) are
evaluated before pattern matching is attempted. In fact, {\it expr}
is a usual \sollya expression, not a pattern.
...
only if they are structurally the same. For example $5 + 1$ only
matches $5 + 1$ and not $1 + 5$, $3 + 3$ or $6$.

This general rule on constant expression admits one exception.
Intervals in \sollya can be defined using constant expressions as
bound but these bounds are immediately evaluated to floating-point
constants. In order to permit pattern matching on intervals, constant
expressions given as bounds of intervals that form patterns are
evaluated before pattern matching. However, in order not conflict with
the rules of no evaluation of variables, these constant expressions as
bounds of intervals in patterns must not contain free variables.
...
\item Patterns like \key{a @ b} are not allowed as they would need to
  perform an ambigous cut of the list or character sequence to be
  ...
  pattern (for example in a list) that would make the cut unambigous.
...
the pattern in the corresponding element of the structure pattern. The
user should be aware that fact that the structure to be matched is
only supposed to have at least the elements of the pattern but that it
may contain more elements is a particular \sollya feature. For
instance with pattern matching, it is hence possible to ensure that
access to particular elements will be possible in a particular code
segment. The following example is meant to clarify this point:
