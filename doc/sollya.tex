...
As usual, the functions \verb|sollya_lib_build_function_libraryfunction|
and \\ \verb|sollya_lib_build_function_libraryfunction_with_data| ``eat
up'' the object {\tt e}, while the functions
\verb|sollya_lib_libraryfunction| and
\verb|sollya_lib_libraryfunction_with_data| do not.
...
In addition to the functionality provided by one of the
\verb|sollya_lib_libraryfunction| and \\
\verb|sollya_lib_build_function_libraryfunction| functions, the \\
\verb|sollya_lib_libraryfunction_with_data| and \\
\verb|sollya_lib_build_function_libraryfunction_with_data| functions
allow for the same binding of an external function to a \sollya object
but additionnally permit an opaque data pointer \verb|data| to be
...
each call, in a additional (last) argument of type \verb|void *| that
the function \verb|f| is supposed to take.

Such opaque data pointers are traditionnally used to distinguish
...
deallocation once the \sollya object built thru invokation of the
...
Section~\ref{lablibraryconstant} for details with respect to
\verb|prec|.
...
procedure-like manner. This functionality is the \sollya library
equivalent to the interactive \com{externalproc} command, described in
Section \ref{labexternalproc}.

The following functions are provided in the \sollya library to bind
external code to a \sollya procedure:
\begin{itemize}
\item To bind a function pointer \verb|p| as a procedure named
  ...
  already taken, the name is unified appending an underscore and a
  unique number to it. The \verb|name| argument is never ``eaten up'',
  i.e. it is up to the user to free any memory allocated for that
  pointer.
  ...
  the function, i.e. it is up to the user to free any memory allocated
...
\item In addition to the basic binding function described above, the \\
  ...
  \sollya object but additionnally permits an opaque data pointer
  ...
  function \verb|p| on each call, in a additional (last) argument of
...
\phantom{\texttt{mmmmmmmmmmmmmmmmmmmmmm}}\verb|sollya_obj_t g)|:\\ works exactly as the previous function but additionnally returns a pointer to the \verb|void *| data field and to the pointer to the deallocation function that had been provided when the library function was created.
...
works exactly as the previous function but additionnally returns a pointer to the \verb|void *| data field and the pointer to the deallocation function that had been provided when the library constant was created.
...
works exactly as the previous function but additionnally returns a pointer to the \verb|void *| data field and the pointer to the deallocation function that had been provided when the external procedure was created.
\end{itemize}
